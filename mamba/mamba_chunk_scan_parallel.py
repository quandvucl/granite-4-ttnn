...
class TensorParallelMamba:
    """Tensor-parallel Mamba2 layer with chunk-scan prefill and fused Metal decode kernels."""
    ...
    def __init__(
        ...
        self._mesh_rows = (
            device.shape[0] if self.is_mesh and self.num_devices > 1 else 1
        )
        ...
        self._ssm_dt_bias = (
            hf_mamba.dt_bias[..., None]
            .expand(hf_mamba.dt_bias.shape[0], hf_mamba.head_dim)
            .contiguous()
        )
        self._ssm_D = (
            hf_mamba.D[..., None]
            .expand(hf_mamba.D.shape[0], hf_mamba.head_dim)
            .contiguous()
        )
    ...
    def _load_weights(self):
        """Load weights related to the Mamba model."""
        ...
        if self.is_mesh and self.num_devices > 1 and self.tensor_parallel:
            ...
            if num_rows >= num_cols:
                tp_mapper = ttnn.ShardTensor2dMesh(
                    self.device, dims=(-1, None), mesh_shape=mesh_shape
                )
        ...
        self._use_col_parallel = (
            self.is_mesh and self.num_devices > 1 and self.tensor_parallel
        )
    ...
    def _preload_decode_constants(self):
        """Upload SSM parameters, conv weights, and pre-allocate output buffers for trace safety."""
        ...
        conv_w_4d = conv_w.T.reshape(1, K, C, 1)
        ...
        # _conv_w_cols used by prefill path (K-loop).
        self._conv_w_cols = [
            to_tt_tensor(
        ...
        self._pad_HD_tt = to_tt_tensor(
            torch.zeros(_max_pad, H, D, dtype=torch.bfloat16).reshape(
                1, _max_pad, H, D
            ),
            self.device,
            ttnn.bfloat16,
        ...
        self._pad_HN_tt = to_tt_tensor(
            torch.zeros(_max_pad, H, N, dtype=torch.bfloat16).reshape(
                1, _max_pad, H, N
            ),
            self.device,
            ttnn.bfloat16,
    ...
    def _all_gather(self, x, dim, cluster_axis, memory_config):
        """All-gather across mesh devices on the given axis, using async CCL if available."""
        ...
        return ttnn.all_gather(
            x, dim=dim, cluster_axis=cluster_axis, memory_config=memory_config
        )

    def forward_prefill_chunk_scan(
        self, hidden_states, cache_params=None, real_seq_len=None
    ):
        """Prefill forward: in_proj -> conv1d -> chunk-scan SSM -> gated norm -> out_proj."""
        replicate_mapper = self.mesh_mapper
        ...
        if isinstance(hidden_states, ttnn.Tensor):
            ...
            seq_len = (
                real_seq_len if real_seq_len is not None else hidden_states.shape[2]
            )
        ...
        if cache_params is not None and hasattr(cache_params, "conv_states"):
            ...
            if self.is_mesh and self.num_devices > 1:
                ...
                tail_cpu = to_torch_tensor(
                    tail_tt, target_shape=(batch_size, tail_len, conv_d)
                )
        ...
        xBC_tt = ttnn.transpose(xBC_tt, 2, 3)

        padded_tt = ttnn.concat(
            [self._prefill_conv_pad_tt, xBC_tt],
            dim=3,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
        ...
        for k in range(kernel_size):
            ...
            term = ttnn.mul(
                s, self._conv_w_cols[k], memory_config=ttnn.DRAM_MEMORY_CONFIG
            )
            s.deallocate(True)
            if acc_tt is None:
                acc_tt = term
        ...
        if self._conv_bias_decode_tt is not None:
            acc_tt = ttnn.add(
                acc_tt, self._conv_bias_decode_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
            )
        ...
        B_raw_tt.deallocate(True)
        C_raw_tt.deallocate(True)
        if group_repeat > 1:
            B_tt = ttnn.reshape(
                ttnn.repeat(
                    ttnn.unsqueeze(B_tt, 3), ttnn.Shape([1, 1, 1, group_repeat, 1])
                ),
                [batch_size, seq_len, H, N],
            )
            C_tt = ttnn.reshape(
                ttnn.repeat(
                    ttnn.unsqueeze(C_tt, 3), ttnn.Shape([1, 1, 1, group_repeat, 1])
                ),
                [batch_size, seq_len, H, N],
            )
        ...
        gated_tt = ttnn.rms_norm(
            gated_tt, epsilon=self._norm_eps, weight=self._norm_weight_tt
        )

        output_tt = ttnn.matmul(gated_tt, self.out_proj_weight_tt)
        gated_tt.deallocate(True)
        if self._use_col_parallel:
            output_tt = self._all_gather(
                output_tt,
                dim=3,
                cluster_axis=self._tp_cluster_axis,
                memory_config=ttnn.DRAM_MEMORY_CONFIG,
    ...
    def _chunk_scan_ssm_ttnn(self, x_tt, B_tt, C_tt, dt_tt, cache_params):
        ...
        dt_tt = ttnn.add(
            dt_tt, self._prefill_dt_bias_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
        )
        ...
        D_residual_tt = ttnn.mul(
            self._prefill_D_tt, x_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
        )

        A_dt_tt = ttnn.mul(
            self._prefill_A_tt, dt_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
        )
        ...
        def _pad_seq(t_tt, extra_cols, preloaded_pad):
            ...
            return ttnn.concat(
                [t_tt, pad], dim=1, memory_config=ttnn.DRAM_MEMORY_CONFIG
            )
        ...
        A_dt_tt = ttnn.permute(A_dt_tt, [0, 3, 1, 2])

        A_cumsum_tt = ttnn.cumsum(A_dt_tt, dim=-1)

        L_tt = ttnn.exp(segment_sum_ttnn(A_dt_tt, self.device))
        ...
        x_T_tt.deallocate(True)
        Bd_tt.deallocate(True)
        x_tt.deallocate(True)

        states_tt = ttnn.reshape(states_tt, [B_sz, C_n, H, Dh, N])

        # Reshape self._ssm_state_tt to [B,1,H,Dh,N] for concat.
        prev_tt = ttnn.reshape(self._ssm_state_tt, [B_sz, 1, H, Dh, N])
        states_tt = ttnn.concat(
            [prev_tt, states_tt], dim=1, memory_config=ttnn.DRAM_MEMORY_CONFIG
        )

        A_cumsum_last_tt = A_cumsum_tt[:, :, :, cs - 1]
        A_cumsum_padded_tt = ttnn.concat(
            [self._seg_zero_col_tt, A_cumsum_last_tt],
            dim=2,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
        ...
        st_tt = ttnn.reshape(
            ttnn.permute(states_tt, [0, 2, 1, 3, 4]), [B_sz * H, C_n + 1, Dh * N]
        )
        ...
        state_decay_tt = ttnn.reshape(
            ttnn.permute(state_decay_tt, [0, 2, 1, 3]), [B_sz * C_n * H, cs, 1]
        )

        st_bch = ttnn.permute(
            ttnn.reshape(states_tt, [B_sz * C_n * H, Dh, N]), [0, 2, 1]
        )
        states_tt.deallocate(True)

        Y_off_tt = ttnn.matmul(C_tt, st_bch, memory_config=ttnn.DRAM_MEMORY_CONFIG)
        C_tt.deallocate(True)
        st_bch.deallocate(True)
        Y_off_tt = ttnn.mul(
            Y_off_tt, state_decay_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
        )
    ...
    def forward(
        ...
        """Dispatch to decode or prefill forward based on sequence length."""
        if isinstance(hidden_states, ttnn.Tensor):
            seq_len = (
                len(cache_position)
    ...
    def _seed_state(self, cache_params=None):
        """Seed conv cache and SSM state from cache_params after prefill."""
    ...
    def _conv1d_decode_tt(self, xBC_tt):
        """Shift-register conv1d decode fallback (no Metal kernel). Updates cols in-place."""
        ...
        if self._conv_bias_decode_tt is not None:
            out = ttnn.add(
                out, self._conv_bias_decode_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
            )
        out = ttnn.silu(out)
        return out
    ...
    def forward_decode(self, hidden_states_tt, cache_params):
        ...
        if self.use_conv1d_kernel:
            ...
            new_cache_tt, conv_out_tt = _conv1d_decode_kernel(
                ...
                self._conv_cache_tt,
                self._conv_w_tt,
                self._conv_bias_decode_tt,
                self.device,
                self._conv_new_cache_tt,
                self._conv_out_tt,
            )
            xBC_tt.deallocate(True)
        ...
        if group_repeat > 1:
            B_tt = ttnn.reshape(
                ttnn.repeat(
                    ttnn.unsqueeze(B_tt, 2), ttnn.Shape([1, 1, group_repeat, 1])
                ),
                [batch_size, H, N],
            )
            C_tt = ttnn.reshape(
                ttnn.repeat(
                    ttnn.unsqueeze(C_tt, 2), ttnn.Shape([1, 1, group_repeat, 1])
                ),
                [batch_size, H, N],
            )
        ...
        dt_tt = ttnn.softplus(dt_tt, beta=1, memory_config=ttnn.DRAM_MEMORY_CONFIG)
        dt_tt = ttnn.clip(
            dt_tt, self.hf_mamba.time_step_limit[0], self.hf_mamba.time_step_limit[1]
        )
        ...
        if self.use_ssm_kernel:
            ...
            hout_tt, y_raw_tt = _ssm_update_kernel(
                ...
                C_tt,
                self.device,
                hout_tt=self._ssm_hout_tt,
                y_tt=self._ssm_y_tt,
                cache_bust_id=self.layer_idx,
            )
            dA_tt.deallocate(True)
            dBx_tt.deallocate(True)
            C_tt.deallocate(True)
            ...
            # Trace-safe: assign new state into the fixed-address buffer
            C_tt = ttnn.unsqueeze(C_tt, -2)
            new_state = ttnn.addcmul(
                dBx_tt, dA_tt, self._ssm_state_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
            )
            dA_tt.deallocate(True)
            dBx_tt.deallocate(True)
            ...
            y_unred = ttnn.mul(
                self._ssm_state_tt, C_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
            )
            C_tt.deallocate(True)
        ...
        y_tt = ttnn.addcmul(
            y_tt, self._ssm_D_tt, x_tt, memory_config=ttnn.DRAM_MEMORY_CONFIG
        )
        x_tt.deallocate(True)
        ...
        output_tt = ttnn.linear(
            scan_tt,
            self.out_proj_weight_decode_tt,
            dtype=ttnn.bfloat16,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
