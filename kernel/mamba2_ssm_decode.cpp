//
// Fused Mamba2 SSM decode-step: collapses multiple Python-dispatched TTNN ops
// into a single C++ call, removing per-op Python dispatch overhead.
//
...
static Tensor unsqueeze_at(const Tensor &t, int dim) {
  ...
  for (uint32_t i = 0; i <= rank; ++i) {
    if (static_cast<int>(i) == pos) sv.push_back(1);
    if (i < rank) sv.push_back(shape[i]);
  }
  return t.reshape(tt::tt_metal::Shape(std::move(sv)));
}

std::pair<Tensor, Tensor> mamba2_ssm_decode(
    const Tensor &dt_raw, const Tensor &dt_bias, const Tensor &x,
    const Tensor &B, const Tensor &C, const Tensor &A, const Tensor &D,
    const Tensor &state_in, float dt_min, float dt_max,
  ...
  Tensor new_state =
      ttnn::operations::ternary::_addcmul(dBx, dA, state_in, 1.0f, mc);
  dA.deallocate(true);
  dBx.deallocate(true);
  ...
  Tensor y = ttnn::operations::reduction::sum(
      yC, std::optional<std::variant<int, int64_t, ttsl::SmallVector<int>>>(-1),
      /*keepdim=*/true, mc);
  yC.deallocate(true);
